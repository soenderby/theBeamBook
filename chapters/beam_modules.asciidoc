...
After the header multiple chunks can be found. The size of each chunk is aligned to a multiple of 4 and each chunk has its own header (shown below).
...
The read/1 function parses the header using the erlang bit syntax. As we can see, the header has the layout described above. Following the header is a number of chunks which are parsed by the read_chunks/2 function. 
This is a recursive function which uses bit syntax to get the name and size of the first chunk. The remaining information, which is both the content of the current chunk and the remaining chunks, is contained in the tail. The size is used to determine the length of the chunk, which is the size aligned by 4 as described above. The length is in turn used to extract the content of the chunk. The name, size, and content of the chunk is added to the accumulator in the tail call.
...
Here `Arity` is hardcoded in the table, which is generated from ops.tab by a genop script when the emulator is built from source.
