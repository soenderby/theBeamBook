...
new register (`x1`). Then it moves the caller saved register (`y0`) to
the first argument register (`x0`). Finally it moves the saved value in
x1 to the caller saved register (`y0`) so that it will survive the next
function call.

Imagine that we would implement three instructions in BEAM called
+move_xx+, +move_yx+, and +move_xy+ (These instructions do not exist
